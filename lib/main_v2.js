...
module.exports = {
  ...
  async rescopePackageToLib() {
    ...
    data.main = data.main.replace('lib/', '');
    ...
    // TODO: lib/package.json should have a repository: "pixelastic/{name}"
    // aberlaas should be removed from devDepencies because added in the root
    // And a author "Tim Carry (@pixelastic)"
    // No bugs field
    // A home page if empty https://projects.pixelastic.com/{name}
    // A license if empty (MIT)

    await writeJson(data, config.libPackage(), { sort: false });
  },
  /**
  ...
  async createMonorepoRoot() {
    const packageTemplate = await template.get('package.json');
    console.info(packageTemplate);
    //   // Write package.json
    //   const template = await this.getTemplate('package.json');
    //   await writeJson(template, this.rootPath('package.json'));
    // Create a root package.json based on template and the one in ./lib
    // Add all root scripts
    // Add Lerna
  },
  /**
   * Create the default root package based on a template
   **/
  async createRootPackage() {
    const templatePackagePath = await template.get('package.json');
    const packageContent = await readJson(templatePackagePath);
    const projectName = config.get('name');

    // Some fields have placeholders to be replaced with the real project name
    const fieldsToReplace = ['name', 'description', 'homepage'];
    _.each(fieldsToReplace, (fieldName) => {
      packageContent[fieldName] = packageContent[fieldName].replace(
        '{name}',
        projectName
      );
    });

    const rootPackagePath = config.rootPath('package.json');
    await writeJson(packageContent, rootPackagePath);
  },
